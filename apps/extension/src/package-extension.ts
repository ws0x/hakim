...
export function packageExtension(): { zipPath: string; sha256: string; manifestVersion: string } {
  ...
  const extRootDir = join(__dirname, "..");
  const manifestPath = join(extRootDir, "manifest.json");
  ...
  function collectFiles(dir: string) {
    ...
    for (const entry of entries) {
      ...
      const relPath = relative(extRootDir, fullPath).replace(/\\/g, "/");
  ...
  const distDir = join(extRootDir, "dist");
